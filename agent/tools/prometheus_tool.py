...
class PrometheusTool:
    ...
    def __init__(self, base_url: str = "http://localhost:9090"):
        ...
        if not self.connected:
            logger.warning(f"⚠️ Could not connect to Prometheus at {base_url}. Will use synthetic data.")
        else:
            logger.info(f"✅ Successfully connected to Prometheus at {base_url}")
    ...
    def query(self, query: str, time: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        """
        if not self.connected:
            logger.warning(f"Not connected to Prometheus. Would have queried: {query}")
    ...
    def get_pod_alerts(self, hours: int = 6, namespace: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        if not self.connected:
            logger.info("Not connected to Prometheus, returning synthetic data")
        ...
        restart_data = self.query_range(restart_query, start_time, end_time, "1m")
        ready_data = self.query_range(ready_query, start_time, end_time, "1m")
        failed_data = self.query_range(failed_query, start_time, end_time, "1m")
        pending_data = self.query_range(pending_query, start_time, end_time, "1m")
        ...
        def process_results(results, issue_type):
            if results.get("status") != "success":
                logger.error(f"Query failed for {issue_type}: {results.get('error', 'Unknown error')}")
                return
                
            result_data = results.get("data", {}).get("result", [])
            logger.info(f"Got {len(result_data)} results for {issue_type} query")
        ...
        if not issue_groups:
            logger.warning("No real issues found in Prometheus. Falling back to synthetic data.")
            return self.generate_synthetic_data()
    ...
    def determine_severity(self, issue_type: str) -> str:
        ...
        """
        high_severity = ["PodCrashLooping", "PodOOMKilled", "PodFailed", "MemoryPressure"]
        medium_severity = ["PodRestarting", "PodNotReady", "ContainerWaiting", "HighCPUUsage"]
